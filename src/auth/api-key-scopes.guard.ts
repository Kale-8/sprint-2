import {
  Injectable, //Inyecta la dependencia.
...
export class ApiKeyScopesGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    if (!apiKey || !keys[apiKey]) {
      //Si no se envio la clave o no existe en la BD, lanza error 401.
      throw new UnauthorizedException('API Key inválida');
    }

    if (requiredScopes) {
      //Verifica si la API Key tiene los scopes necesarios.
      const hasScopes = requiredScopes.every(
        (
          scope, //verifica que todos los scopes requeridos estén en los scopes de la API Key
