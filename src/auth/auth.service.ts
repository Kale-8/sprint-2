...
export class AuthService {
  ...
  async generateTokens(usuario: Usuario) {
    //Funcion para generar tokens JWT con el usuario validado
    const payload = {
      //Guardamos en el payload el id, email y role del usuario
      sub: usuario.id,
      email: usuario.email,
    ...
    const accessToken = this.jwtService.sign(payload, {
      //this.jwtService.sign genera el token con el payload con su clave secreta y tiempo de expiracion.
      secret: process.env.JWT_SECRET,
      expiresIn: '15m',
    });

    const refreshToken = this.jwtService.sign(payload, {
      //Generamos el refresh token con mayor tiempo de expiracion con su respectiva clave secreta.
      secret: process.env.JWT_REFRESH_SECRET,
      expiresIn: '7d',
    });

    return {
      //Retornamos ambos tokens que seran utilizados para darle permiso de acceso al usuario dependiendo del rol.
      accessToken,
      refreshToken,
    };
  }
