...
export class LoggingInterceptor implements NestInterceptor {
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> { //context da acceso a la peticion, next permite continuar la solicitud y observable devuelve la respuesta.
    ...
    return next.handle().pipe(   //Ejecuta el controlador y obtiene la respuesta como un observable.
      tap(() => {    //Ejecuta una acción secundaria cuando la respuesta está lista.
