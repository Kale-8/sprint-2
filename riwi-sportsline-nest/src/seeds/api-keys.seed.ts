...
export async function seedApiKeys(dataSource: DataSource) {
  ...
  const adminUser = await userRepository.findOne({
    where: { email: 'admin@sportsline.com' },
  });
  ...
  let testKey = await apiKeyRepository.findOne({
    where: { nombre: 'Test API Key' },
  });
  if (!testKey) {
  ...
  let readOnlyKey = await apiKeyRepository.findOne({
    where: { nombre: 'Read-Only API Key' },
  });
  if (!readOnlyKey) {
  ...
  let tempKey = await apiKeyRepository.findOne({
    where: { nombre: 'Temporary API Key' },
  });
  if (!tempKey) {
    ...
    console.log(
      `  ✓ Created Temporary API Key (expires in 30 days): ${tempKeyPlain}`,
    );
